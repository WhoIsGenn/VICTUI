...
function vict:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Title = NotifyConfig.Title or "VictoriaHub"
...
function notif(msg, delay, color, title, desc)
    return vict:MakeNotify({
        Title = title or "VictoriaHub",
        Description = desc or "Notification",
        Content = msg or "Content",
        Color = color or Color3.fromRGB(120, 200, 255),
        Delay = delay or 4
    })
end
...
function vict:Window(GuiConfig)
    ...
    GuiConfig.Title        = GuiConfig.Title or "VictoriaHub"
    GuiConfig.Footer       = GuiConfig.Footer or "VictoriaHub >:D"
    GuiConfig.Color        = GuiConfig.Color or Color3.fromRGB(255, 0, 255)
    GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 120
    GuiConfig.Version      = GuiConfig.Version or 1
    GuiConfig.Icon         = GuiConfig.Icon or "rbxassetid://105950071843685"
    ...
    local vict = Instance.new("ScreenGui");
    local DropShadowHolder = Instance.new("Frame");
    local DropShadow = Instance.new("ImageLabel");
    ...
    vict.Name = "vict"
    vict.ResetOnSpawn = false
    vict.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = vict

    DropShadowHolder.Position = UDim2.new(0, (vict.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (vict.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("vict") then
            vict:Destroy()
        end
    end
