...
local function OptimizeInstance(Inst)
    ...
    pcall(function()
        ...
        -- MeshParts (with full error protection)
        elseif className == "MeshPart" then
            -- Skip if locked
            if Inst.Locked then return end
            
            -- Try to modify with individual pcalls
...
local function ProcessObjectsInBatches()
    ProcessThread = task.spawn(function()
        ...
        local processed = 0
        local skipped = 0
        local batchSize = 5 -- Even smaller batch
        
        
        
        -- Process in tiny batches with frequent yields
        for i = 1, totalObjects, batchSize do
            ...
            for j = i, endIndex do
                ...
                if obj and obj.Parent then
                    -- Super strict filter - only optimize what matters for FPS
                    local cn = obj.ClassName
                    if cn == "MeshPart" or cn == "SpecialMesh" or cn == "Decal" or cn == "Texture"
                    or cn == "ParticleEmitter" or cn == "Trail" or cn == "Smoke" 
                    or cn == "Fire" or cn == "Sparkles" or cn == "Sky" 
                    or cn == "Shirt" or cn == "Pants" or cn == "ShirtGraphic" or cn == "SurfaceAppearance" then
            ...
            -- Yield EVERY batch for maximum smoothness
            task.wait()
            
            -- Progress update every 1000 objects
            if (processed + skipped) % 1000 == 0 then
