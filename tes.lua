...
local blatantBETASection = Tab3:AddSection("Blatant Control")

--====================================
-- REMOTES
--====================================
local RF_Charge   = net["RF/ChargeFishingRod"]
local RF_Request  = net["RF/RequestFishingMinigameStarted"]
local RF_Cancel   = net["RF/CancelFishingInputs"]
local RE_Complete = net["RF/CatchFishCompleted"]
...
blatantBETASection:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num > 0 then
            blatant.completeDelay = num
            notif("Complete Delay: " .. num .. "s", 2, Color3.fromRGB(255, 170, 0))
        end
    end
})
...
blatantBETASection:AddInput({
    ...
    Callback = function(v)
        local num = tonumber(v)
        if num and num > 0 then
            blatant.cancelDelay = num
            notif("Cancel Delay: " .. num .. "s", 2, Color3.fromRGB(255, 170, 0))
        end
    end
})
...
local function StartBlatantLoop()
    if blatant.thread then 
        task.cancel(blatant.thread) 
        blatant.thread = nil
    end

    blatant.thread = task.spawn(function()
        local cycleCount = 0
        
        while blatant.enabled do
            cycleCount = cycleCount + 1
            
            -- Step 1: START FISHING
            ForceStep123()
            
            -- Wait for complete delay (FIXED - simple wait)
            task.wait(blatant.completeDelay)
            
            if not blatant.enabled then break end
            
            -- Step 2: COMPLETE
            ForceStep4()
            
            -- Wait for cancel delay (FIXED - simple wait)
            task.wait(blatant.cancelDelay)
            
            if not blatant.enabled then break end
            
            -- Reset setiap 20 cycles (reduced frequency)
...
blatantBETASection:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            -- LEMPAR PERTAMA LANGSUNG & SYNC (TANPA TASK.SPAWN)
            pcall(function()
                RF_Cancel:InvokeServer()        
                RF_Charge:InvokeServer({ [1] = os.clock() })
                RF_Request:InvokeServer(os.clock(), os.clock(), os.clock())
            ...
            blatant.thread = task.spawn(function()
                while blatant.enabled do
                    ...
                    -- LEMPAR LAGI (dalam thread, tapi pertama udah langsung)
                    task.spawn(function()
                        pcall(function()
                            RF_Cancel:InvokeServer()        
                            RF_Charge:InvokeServer({ [1] = os.clock() })
            ...
            notif("Blatant: ON", 1, Color3.fromRGB(0, 255, 0))
        else
            if blatant.thread then 
                task.cancel(blatant.thread) 
                blatant.thread = nil 
            ...
            task.spawn(ForceCancel)
            notif("Blatant: OFF", 1, Color3.fromRGB(255, 0, 0))
        end
    end
})
...
-- BLATANT V3 SECTION (Chloex Style)
local blatantV3Section = Tab3:AddSection("Blatant V3")
...
local V3 = {
    ...
    -- DUA DELAY TERPISAH
    throwDelay = 0.3,   -- Delay antara lempar (bait)
    completeDelay = 0.7, -- Delay antara complete (cast)
    
    -- Counter untuk pattern
    throwCount = 0
}

-- Throw Delay (Bait Delay)
blatantV3Section:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num > 0 then
            V3.throwDelay = num
            notif("Throw Delay: " .. num .. "s", 2, Color3.fromRGB(0, 255, 0))
        end
    end
})

-- Complete Delay (Cast Delay)  
blatantV3Section:AddInput({
    Title = "Complete Delay",
    Default = "0.7",
    Callback = function(v)
        ...
        if num and num > 0 then
            ...
            notif("Complete Delay: " .. num .. "s", 2, Color3.fromRGB(0, 255, 0))
        end
    end
})
...
blatantV3Section:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            ...
            -- Stop threads lama
            if V3.throwThread then task.cancel(V3.throwThread) end
            if V3.completeThread then task.cancel(V3.completeThread) end
            
            -- ===== THREAD 1: LEMPAR TERUS (cepat) =====
            V3.throwThread = task.spawn(function()
                while V3.running do
                    V3.throwCount += 1
                    
                    -- Pattern berdasarkan throwCount
                    if V3.throwCount % 3 == 1 then
                        -- Throw 1: dengan cancel awal
                        pcall(function()
                            net["RF/CancelFishingInputs"]:InvokeServer()
                            net["RF/ChargeFishingRod"]:InvokeServer(tick())
                            net["RF/RequestFishingMinigameStarted"]:InvokeServer(-1.233184814453125, 0.9965292652011685)
                        end)
                    elseif V3.throwCount % 3 == 2 then
                        -- Throw 2: setelah cancel 1
                        task.wait(0.08) -- Cancel delay internal
                        pcall(function()
                            net["RF/CancelFishingInputs"]:InvokeServer()
                            net["RF/ChargeFishingRod"]:InvokeServer(tick())
                            net["RF/RequestFishingMinigameStarted"]:InvokeServer(-1.233184814453125, 0.9965292652011685)
                    ...
                    -- Tunggu throw delay
                    task.wait(V3.throwDelay)
                end
            end)
            
            -- ===== THREAD 2: COMPLETE TERUS (lambat) =====
            V3.completeThread = task.spawn(function()
                task.wait(0.5) -- Tunggu awal biar ga langsung complete
                ...
                while V3.running do
                    ...
                    -- Tunggu complete delay (lebih lambat dari throw)
                    task.wait(V3.completeDelay)
                end
            end)
            
            notif("V3: Enabled", 2, Color3.fromRGB(0, 255, 0))
            notif("Throw: " .. V3.throwDelay .. "s | Complete: " .. V3.completeDelay .. "s", 3, Color3.fromRGB(0, 170, 255))
            
        else
            -- Stop semua threads
            if V3.throwThread then
                task.cancel(V3.throwThread)
                V3.throwThread = nil
            end
            if V3.completeThread then
                task.cancel(V3.completeThread)
                V3.completeThread = nil
            end
            ...
            notif("V3: Disabled", 2, Color3.fromRGB(255, 0, 0))
        end
    end
})
