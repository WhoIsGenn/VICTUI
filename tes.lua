...
local function bindChar(c)
    ...
    if c and c:FindFirstChild("HumanoidRootPart") then
        ST.hrp = c.HumanoidRootPart
    end
end

if ST.player.Character then
    bindChar(ST.player.Character)
end
...
local function updateCache()
    local now = tick()
    if now - ST.lastCacheUpdate > ST.cacheUpdateInterval then
        ST.descendantsCache = workspace:GetDescendants()
        ST.lastCacheUpdate = now
    end
end

-- Force TP function
local function forceTP(pos)
    if not ST.hrp or not ST.hrp.Parent then return end
    ...
    pcall(function()
        ...
        if ST.hrp:FindFirstChild("BodyVelocity") then
            ST.hrp.BodyVelocity:Destroy()
        end
    end)
...
local function findEvent(cfg)
    ...
    if cfg.TargetName == "Model" then
        ...
        -- Use cached descendants
        updateCache()
        
        for _, d in ipairs(ST.descendantsCache) do
            if d and d.Parent and d.Name == cfg.TargetName then
                local pos = nil
                ...
                if pos then
                    for _, loc in ipairs(cfg.Locations) do
                        if (pos - loc).Magnitude <= ST.megRadius then
                            found = pos
                            break
                        end
                    end
...
local function runEventTP()
    while ST.autoTP and task.wait(ST.tpCooldown) do
        if not ST.hrp or not ST.hrp.Parent then
            task.wait(1)
            continue
        end
        ...
        -- Find and TP to events
        for _, cfg in ipairs(list) do
            if not ST.autoTP then break end
            
            local found = findEvent(cfg)
            
            if found then
                forceTP(found)
                break -- Only TP to highest priority event found
            end
        end
    end
end

-- Float on water with error handling
...
local function setupFloat()
    ...
    floatConnection = game:GetService("RunService").Heartbeat:Connect(function()
        if not ST.autoFloat or not ST.hrp or not ST.hrp.Parent then return end
        
        pcall(function()
            local pos = ST.hrp.Position
            local waterLevel = 0
            
            -- Try to get water level safely
            if workspace.Terrain then
                local region = Region3.new(pos - Vector3.new(4, 4, 4), pos + Vector3.new(4, 4, 4))
            ...
            -- Default water level if terrain water not found
            if waterLevel == 0 then
                waterLevel = -1.4 -- Default based on your locations
            end
            
            local targetY = waterLevel + ST.floatOffset
            
            if pos.Y < targetY then
                ST.hrp.CFrame = CFrame.new(pos.X, targetY, pos.Z)
                ST.hrp.AssemblyLinearVelocity = Vector3.new(ST.hrp.AssemblyLinearVelocity.X, 0, ST.hrp.AssemblyLinearVelocity.Z)
            end
        end)
    end)
end
...
ST.player.CharacterRemoving:Connect(function()
    ST.hrp = nil
    ST.char = nil
end)
...
-- ULTRA FPS BOOST (Optimized Non-Freeze Version)
...
local function PartOfCharacter(Inst)
    if ME.Character and Inst:IsDescendantOf(ME.Character) then
        return true
    end
    return false
end
...
local function OptimizeInstance(Inst)
    if PartOfCharacter(Inst) or Inst:IsDescendantOf(Players) then 
        return 
    end
    ...
    pcall(function()
        ...
        -- Clothing
        elseif className == "Shirt" or className == "Pants" or className == "ShirtGraphic" or className == "SurfaceAppearance" then
            Inst:Destroy()
            
        -- Parts
        elseif className == "Part" or className == "UnionOperation" then
...
local function ProcessObjectsInBatches()
    ProcessThread = task.spawn(function()
        ...
        local processed = 0
        local batchSize = 25 -- Very small batch to prevent freeze
        
        print("[FPS Boost] Found " .. totalObjects .. " objects to process")
        
        -- Process in small batches
        for i = 1, totalObjects, batchSize do
            if not FPSBoostEnabled then 
                print("[FPS Boost] Stopped early")
                break 
            end
            
            local endIndex = math.min(i + batchSize - 1, totalObjects)
            
            -- Process batch
            for j = i, endIndex do
                if allObjects[j] and allObjects[j].Parent then
                    OptimizeInstance(allObjects[j])
                    processed = processed + 1
                end
            end
            
            -- Yield to prevent freeze - CRITICAL
            RunService.Heartbeat:Wait()
        end
        
        print("[FPS Boost] Complete! Optimized " .. processed .. "/" .. totalObjects .. " objects")
    end)
end
...
local function StartFPSBoost()
    ...
    -- Apply instant environment settings (no freeze)
    ApplyEnvironmentSettings()
    
    print("[FPS Boost] Environment optimized instantly")
    print("[FPS Boost] Starting background object optimization...")
    
    -- Process objects in background (with yields)
...
local function StopFPSBoost()
    ...
    print("[FPS Boost] Stopped")
end

graphicsSection:AddToggle({
    Title = "FPS Boost (No Freeze)",
    Default = false,
    Callback = function(v)
        if v then
            StartFPSBoost()
