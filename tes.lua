...
-- BLATANT V3 SECTION (Persis V2 Style)
...
blatantV3Section:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            if V3.thread then task.cancel(V3.thread) end
            
            V3.thread = task.spawn(function()
                while V3.running do
                    -- INI YANG BEDA: pake task.spawn di dalam seperti V2
                    task.spawn(function()
                        -- Throw 1
                        pcall(function()
                            net["RF/CancelFishingInputs"]:InvokeServer()
                            ...
                            -- Cancel 1 (dengan delay internal)
                            task.wait(0.08)
                            net["RF/CancelFishingInputs"]:InvokeServer()
                            ...
                            -- Complete dengan delay dari input
                            task.wait(V3.completeDelay)
                            net["RE/FishingCompleted"]:FireServer()
                        end)
                    end)
                    
                    -- Tunggu reel delay sebelum next cycle (PERSIS V2)
                    task.wait(math.max(V3.reelDelay))
                end
            end)
