...
-- BLATANT V2 SECTION
local blatantV2Section = Tab3:AddSection("Blatant Unstable")
...
-- ================================
-- BLATANT V2 AGGRESSIVE (REAL FIX)
-- ================================
local blatantV3Section = Tab3:AddSection("Blatant V2 Aggressive")
...
-- AUTO TUNE (perfect tapi random)
local function AutoTune()
    local power = -139.63 + math.random(-20,20)/10
    local acc = 0.996 + math.random(0,4)/1000
    return power, acc
end

blatantV3Section:AddToggle({
    Title = "Enable Blatant V2 (Aggressive)",
    Default = false,
    Callback = function(v)
        State.Running = v

        if v then
            if mainTask then task.cancel(mainTask) end
            if equipTask then task.cancel(equipTask) end
                ...
                while State.Running do
                    ...
                    task.spawn(function()
                        pcall(function()
                            ...
                            local p,a = AutoTune()
                            net["RF/RequestFishingMinigameStarted"]:InvokeServer(p,a)
                        end)
                    end)
