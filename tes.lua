...
-- Load DataService dan ItemUtility
task.spawn(function()
    Trade.ItemUtility = require(game:GetService("ReplicatedStorage").Shared.ItemUtility)
    local Replion = require(game:GetService("ReplicatedStorage").Packages.Replion)
    Trade.DataService = Replion.Client:WaitReplion("Data")
    
    -- Buat UI setelah load
    task.wait(1)
    createTradeUI()
end)
...
-- Function untuk get fish dari inventory
local function getFishFromInventory()
    ...
    if items and typeof(items) == "table" then
        for _, itemData in pairs(items) do
            local itemInfo = Trade.ItemUtility.GetItemDataFromItemType("Items", itemData.Id)
            if itemInfo and itemInfo.Data and itemInfo.Data.Type == "Fish" then
                local fishName = itemInfo.Data.Name or "Unknown Fish"
                if not table.find(fishList, fishName) then
...
-- Function untuk create UI
local function createTradeUI()
    -- Player Dropdown
    local playerList = getPlayerList()
    local defaultPlayer = #playerList > 0 and playerList[1] or ""
    ...
    -- Fish Dropdown
    local fishList = getFishFromInventory()
    tradeSection:AddDropdown({
        Title = "Select Fish",
        Options = fishList,
        Default = {},
    ...
    tradeSection:AddButton({
        ...
        Callback = function()
            ...
            if Trade.SelectedPlayer ~= "" and not game.Players:FindFirstChild(Trade.SelectedPlayer) then
                if #playerList > 0 then
                    Trade.SelectedPlayer = playerList[1]
                else
...
game.Players.PlayerAdded:Connect(function()
    task.wait(2)
    if Trade.AutoTradeEnabled and Trade.SelectedPlayer == "" then
        local playerList = getPlayerList()
        if #playerList > 0 then
            Trade.SelectedPlayer = playerList[1]
        end
...
game.Players.PlayerRemoving:Connect(function(player)
    if Trade.AutoTradeEnabled and Trade.SelectedPlayer == player.Name then
        task.wait(1)
        local playerList = getPlayerList()
        if #playerList > 0 then
            Trade.SelectedPlayer = playerList[1]
