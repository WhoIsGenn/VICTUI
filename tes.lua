...
-- AUTO TRADE SECTION
local tradeSection = Tab3:AddSection("Auto Trade")
...
-- Load fish data langsung (seperti auto favorite)
task.spawn(function()
...
tradeSection:AddDropdown({
    ...
    Default = #playerOptions > 0 and playerOptions[1] or "",
    Callback = function(v)
        ...
        if state then
            task.spawn(function()
                while Trade.AutoTradeEnabled do
                    -- Load services di dalam loop (seperti auto totem)
