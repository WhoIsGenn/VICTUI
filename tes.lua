...
-- ULTRA FPS BOOST (Aggressive Mode)
...
local function isPlayerRelated(obj)
    ...
    if obj:IsA("BackpackItem") or obj:FindFirstAncestorWhichIsA("BackpackItem") then
        return true
    end
...
local function optimizeInstance(obj)
    if ProcessedInstances[obj] or isPlayerRelated(obj) then return end
    ...
    pcall(function()
        ...
        elseif obj:IsA("Explosion") then
            saveOriginal(obj, "BlastPressure")
            saveOriginal(obj, "BlastRadius")
            saveOriginal(obj, "Visible")
            
            obj.BlastPressure = 1
            obj.BlastRadius = 1
            obj.Visible = false
        elseif obj:IsA("Clothing") or obj:IsA("ShirtGraphic") or obj:IsA("SurfaceAppearance") then
            obj:Destroy()
...
local function applyEnvironmentBoost()
    ...
    saveOriginal(Lighting, "ShadowSoftness")
    ...
    pcall(function()
        ...
        saveOriginal(MaterialService, "Use2022Materials")
...
local function applyBoost()
    -- Apply environment first (instant)
    applyEnvironmentBoost()
    
    -- Optimize objects in batches (non-blocking)
    task.spawn(function()
        local services = {
            workspace,
        ...
        -- Process each service separately with yields
        for _, service in ipairs(services) do
            if not FPSBoost then break end
            
            local descendants = service:GetDescendants()
            local batchSize = 50 -- Smaller batch for smoother performance
            
            for i = 1, #descendants, batchSize do
                if not FPSBoost then break end
                
                local endIndex = math.min(i + batchSize - 1, #descendants)
                
                for j = i, endIndex do
                    local obj = descendants[j]
                    
                    -- Quick type check before processing
                    local objType = obj.ClassName
                    if objType == "MeshPart" or objType == "Part" or objType == "UnionOperation" 
                    or objType == "SpecialMesh" or objType == "Decal" or objType == "Texture"
                    or objType == "ParticleEmitter" or objType == "Trail" or objType == "Smoke" 
                    or objType == "Fire" or objType == "Sparkles" or objType == "Sky" then
                        optimizeInstance(obj)
                        processedCount = processedCount + 1
                    end
                end
                
                -- Yield more frequently for smoother experience
                task.wait()
            end
        end
        
        print("[FPS Boost] Optimization complete! Processed " .. processedCount .. " objects.")
    end)
...
local function restoreBoost()
    ...
    -- Restore values in batches
    task.spawn(function()
        local objects = {}
        for obj in pairs(OriginalValues) do
        ...
        for i = 1, totalObjects, batchSize do
            ...
            for j = i, endIndex do
                ...
                if obj and obj.Parent then
                    for property, value in pairs(OriginalValues[obj]) do
                        pcall(function()
                            obj[property] = value
                        end)
                    end
        ...
        print("[FPS Boost] Restoration complete!")
    end)
...
graphicsSection:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            applyBoost()
            print("[FPS Boost] Optimizing objects in background...")
        else
            restoreBoost()
            print("[FPS Boost] Restoring original settings...")
        end
    end
})
