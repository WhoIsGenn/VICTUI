...
-- Function untuk get player list (SAMA seperti player teleport)
...
task.spawn(function()
    for _, item in pairs(game:GetService("ReplicatedStorage").Items:GetChildren()) do
        local ok, data = pcall(require, item)
        if ok and data.Data and data.Data.Type == "Fish" then
            table.insert(Trade.FishNames, data.Data.Name)
        end
    end
    table.sort(Trade.FishNames)
end)

-- Player Dropdown (LANGSUNG LOAD - TANPA SETOPTIONS)
        ...
        if state then
            task.spawn(function()
                while Trade.AutoTradeEnabled do
                    -- REFRESH PLAYER LIST DULU SEBELUM TRADE
                    ...
                    -- Jika player tidak ditemukan, pilih player pertama
                    ...
                    -- Lanjut trade jika ada player target
...
-- Refresh Player Button (VERSI FIX - TANPA SETOPTIONS)
tradeSection:AddButton({
    ...
    Callback = function()
        ...
        -- Variable Trade.SelectedPlayer sudah di-update
    end
})

-- Auto refresh ketika player join/leave (SAMA seperti player teleport)
game.Players.PlayerAdded:Connect(function()
    task.wait(2)
    if Trade.AutoTradeEnabled then
        if Trade.SelectedPlayer == "" then
            local playerList = getPlayerList()
...
game.Players.PlayerRemoving:Connect(function(player)
    if Trade.SelectedPlayer == player.Name then
        task.wait(1)
        local playerList = getPlayerList()
        if #playerList > 0 then
            Trade.SelectedPlayer = playerList[1]
...
-- WEBHOOK SECTION
local webhookSection = Tab0:AddSection("Webhook")
...
local function sendNewFishWebhook(newlyCaughtFish)
    ...
    pcall(function()
        ...
        print("Fish webhook sent:", newFishDetails.Name)
    end)
end

-- Build database on start
task.spawn(function()
    buildFishDatabase()
end)
...
task.spawn(function()
    while true do
        ...
        if webhookEnabled and DataService then
...
-- Function to get player list (SAMA DENGAN AUTO TRADE)
local function getPlayerList()
    local players = {}
    for _, plr in ipairs(game.Players:GetPlayers()) do
        table.insert(players, plr.Name)
