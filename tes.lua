...
-- AUTO EQUIP ROD FUNCTION YANG BENER
local function EquipRod()
    safeCall("equip_rod", function()
...
-- FUNCTION UNTUK CHECK ROD TERUS MENERUS
local function autoEquipRodLoop()
    while autoEquipRodEnabled do
        -- Check apakah rod sudah di tangan
        local player = game.Players.LocalPlayer
        local character = player.Character
        local hasRod = false
        
        if character then
            -- Cek tools di character (tangan)
            for _, item in ipairs(character:GetChildren()) do
                if item:IsA("Tool") and (item.Name:find("Fishing") or item.Name:find("Rod")) then
                    hasRod = true
                    break
                end
            end
            
            -- Jika tidak ada rod di tangan, coba equip
            if not hasRod and autoEquipRodEnabled then
                EquipRod()
            end
        end
        
        -- Tunggu 2 detik sebelum check lagi
        task.wait(2)
    end
end
...
fishingSection:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            -- Start loop untuk check terus
            if autoEquipThread then
                task.cancel(autoEquipThread)
            end
            
            autoEquipThread = task.spawn(autoEquipRodLoop)
...
local function SetupRespawnEvents()
    local player = game.Players.LocalPlayer
...
task.spawn(SetupRespawnEvents)

-- FISHING FUNCTIONS (sisanya tetap sama...)
...
-- FAST REEL SECTION (dengan auto equip)
local fastReelSection = Tab3:AddSection("Fast Reel [BETA]")

local fastReelActive = false
local fastReelThread = nil
local FastReelDelay = 0.8

fastReelSection:AddInput({
    Title = "Complete Reel",
    Default = "0.8",
    Callback = function(v)
        local num = tonumber(v)
        if num and num >= 0.3 then
            FastReelDelay = num
            notif("Complete Reel: " .. num .. "s", 2, Color3.fromRGB(0, 255, 170))
        end
    end
})
...
local function fastReelLoop()
    while fastReelActive do
        ...
        task.spawn(function()
            pcall(function()
                net["RF/RequestFishingMinigameStarted"]:InvokeServer(-139.63, 0.996, tick())
            end)
        end)
        task.wait(0.03)
        ...
        task.wait(FastReelDelay)
        
        if not fastReelActive then break end
...
fastReelSection:AddToggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            if fastReelThread then
                task.cancel(fastReelThread)
            end
            
            fastReelThread = task.spawn(function()
                while fastReelActive do
                    local success, err = pcall(fastReelLoop)
                    if not success and fastReelActive then
                        warn("Fast Reel loop error:", err)
                    end
                end
            end)
            
            notif("Fast Reel: ON", 2, Color3.fromRGB(0, 255, 0))
        else
            ...
            notif("Fast Reel: OFF", 2, Color3.fromRGB(255, 0, 0))
        end
    end
})

-- BLATANT SECTION (dengan auto equip)
local blatantSection = Tab3:AddSection("Blatant Mode [BETA]")

local blatantActive = false
local blatantThread = nil
local CycleDelay = 0.3
local BlatantDelay = 0.7
...
blatantSection:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num >= 0.3 then
            CycleDelay = num
            notif("Cycle Delay: " .. num .. "s", 2, Color3.fromRGB(0, 255, 170))
        end
    end
})
...
blatantSection:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num >= 0.3 then
            BlatantDelay = num
            notif("Complete Delay: " .. num .. "s", 2, Color3.fromRGB(0, 255, 170))
        end
    end
})
...
local function blatantLoop()
    while blatantActive do
        ...
        if not blatantActive then break end
    ...
    Callback = function(state)
        ...
        if state then
            ...
            blatantThread = task.spawn(function()
                while blatantActive do
                    local success, err = pcall(blatantLoop)
                    if not success and blatantActive then
                        warn("Blatant loop error:", err)
                    end
                end
            end)
            
            notif("Blatant: ON", 2, Color3.fromRGB(0, 255, 0))
        else
            blatantActive = false
            ...
            notif("Blatant: OFF", 2, Color3.fromRGB(255, 0, 0))
        end
    end
})

-- RECOVERY FISHING BUTTONS
fastReelSection:AddButton({
...
-- =========================
-- AUTO TRADE SECTION (REWRITE - SAMA KAYA AUTO FAVORITE)
-- =========================
local tradeSection = Tab3:AddSection("Auto Trade Settings")

-- =========================
-- GLOBAL STATE (SIMPAN SEMUA DATA)
-- =========================
local GlobalTrade = {
...
-- =========================
-- INITIALIZE DATA (SAMA KAYA AUTO FAVORITE)
-- =========================
local Players = game:GetService("Players")
local RS = game:GetService("ReplicatedStorage")
local LocalPlayer = Players.LocalPlayer

-- Load player list (langsung update saat join/leave)
local function updatePlayerList()
    GlobalTrade.PlayerNames = {}
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            table.insert(GlobalTrade.PlayerNames, plr.Name)
        end
    end
    table.sort(GlobalTrade.PlayerNames)
end

-- Load fish dari inventory (async)
task.spawn(function()
    -- Load ItemUtility
    local ok1, itemUtil = pcall(function()
        return require(RS.Shared.ItemUtility)
    end)
    
    if ok1 and itemUtil then
        GlobalTrade.ItemUtility = itemUtil
    end
    
    -- Load DataService
    local ok2, replion = pcall(function()
        return require(RS.Packages.Replion)
    end)
    
    if ok2 and replion then
        GlobalTrade.DataService = replion.Client:GetReplion("Data")
        
        -- Load fish data setelah DataService ready
        if GlobalTrade.DataService then
            task.wait(1)  -- Tunggu sebentar
            
            local items = GlobalTrade.DataService:GetExpect({"Inventory", "Items"})
            if items and typeof(items) == "table" then
                GlobalTrade.FishNames = {}
                
                for _, itemData in pairs(items) do
                    local itemInfo = GlobalTrade.ItemUtility.GetItemDataFromItemType("Items", itemData.Id)
                    if itemInfo and itemInfo.Data and itemInfo.Data.Type == "Fish" then
                        local fishName = itemInfo.Data.Name or "Unknown Fish"
                        if not table.find(GlobalTrade.FishNames, fishName) then
                            table.insert(GlobalTrade.FishNames, fishName)
                        end
                    end
                end
                
                table.sort(GlobalTrade.FishNames)
                
                -- Auto update fish dropdown jika sudah dibuat
                if GlobalTrade.FishDropdown then
                    GlobalTrade.FishDropdown:SetOptions(GlobalTrade.FishNames)
                end
            end
        end
    end
...
-- =========================
-- UI CREATION (SEKALI SAJA - SAMA KAYA AUTO FAVORITE)
-- =========================

-- 1. Player Dropdown
updatePlayerList()  -- Update dulu
GlobalTrade.SelectedPlayer = GlobalTrade.PlayerNames[1] or ""

local playerDropdown = tradeSection:AddDropdown({
    Title = "Select Player",
    Options = GlobalTrade.PlayerNames,
    Default = GlobalTrade.PlayerNames[1] or "No Players",
    Callback = function(v)
        GlobalTrade.SelectedPlayer = v
    end
})

-- 2. Fish Dropdown (Multi select - SAMA KAYA AUTO FAVORITE)
GlobalTrade.FishDropdown = tradeSection:AddDropdown({
    Title = "Select Fish",
    Options = {"Loading fish data..."},
    Default = {},
    Multi = true,
    Callback = function(v)
        GlobalTrade.SelectedFish = v
    end
})
...
tradeSection:AddInput({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n and n >= 1 then 
            GlobalTrade.TradeDelay = n 
        end
    end
})

-- 4. Auto Trade Toggle (SAMA STRUCTURE KAYA AUTO FAVORITE)
tradeSection:AddToggle({
    Title = "Auto Trade",
    Default = false,
    Callback = function(state)
        GlobalTrade.AutoTradeEnabled = state
        
        if state then
            notif("Auto Trade: ON", 3)
            startAutoTrade()
        else
            notif("Auto Trade: OFF", 3)
        end
    end
})

-- =========================
-- TRADING LOGIC (SEPARATE FUNCTION)
-- =========================
local function sendTrade(targetPlayer, itemUUID)
    if not targetPlayer then return false end
    
    -- Cek remote availability
    if not net["RF/CanSendTrade"] or not net["RF/InitiateTrade"] then
        return false
    end
    
    -- Cek apakah bisa trade dengan player ini
    ...
    -- Initiate trade dengan item UUID
    local tradeSuccess = pcall(function()
        net["RF/InitiateTrade"]:InvokeServer(targetPlayer.UserId, itemUUID)
        return true
    end)
...
-- =========================
-- AUTO TRADE MAIN LOOP (SAMA PATTERN KAYA AUTO FAVORITE)
-- =========================
local function startAutoTrade()
    task.spawn(function()
        while GlobalTrade.AutoTradeEnabled do
            -- Cek target player masih online
            local target = Players:FindFirstChild(GlobalTrade.SelectedPlayer)
            if not target then
                task.wait(GlobalTrade.TradeDelay)
                continue
            end
            
            -- Cek services ready
            if not (GlobalTrade.DataService and GlobalTrade.ItemUtility) then
                task.wait(GlobalTrade.TradeDelay)
                continue
            end
            ...
            -- Get inventory items
            local items = GlobalTrade.DataService:GetExpect({"Inventory", "Items"})
            if not items or typeof(items) ~= "table" then
                task.wait(GlobalTrade.TradeDelay)
                continue
            end
            ...
            for _, itemData in pairs(items) do
                if not GlobalTrade.AutoFavoriteEnabled then break end
                ...
                if itemInfo and itemInfo.Data and itemInfo.Data.Type == "Fish" then
                    ...
                    -- Filter logic (SAMA KAYA AUTO FAVORITE)
                    local shouldTrade = false
                    
                    if #GlobalTrade.SelectedFish == 0 then
                        -- Jika tidak ada fish yang dipilih, trade semua
                        shouldTrade = true
                    else
                        -- Cek apakah fish ini ada di selected list
                        for _, selectedFish in ipairs(GlobalTrade.SelectedFish) do
                            if selectedFish == fishName then
                                shouldTrade = true
                                break
                            end
                    ...
                    -- Trade jika lolos filter dan punya UUID
                    if shouldTrade and uuid then
                        local success = sendTrade(target, uuid)
                        
                        if success then
                            tradedCount = tradedCount + 1
                            notif("Traded: " .. fishName, 1)
                            task.wait(GlobalTrade.TradeDelay)
                        else
                            break  -- Stop jika trade gagal
                        end
                    end
                end
            end
            ...
            if tradedCount == 0 then
                -- Jika tidak ada fish yang ditrade, tunggu lebih lama
                task.wait(GlobalTrade.TradeDelay * 2)
            else
                task.wait(GlobalTrade.TradeDelay)
            end
        end
...
-- =========================
-- ADDITIONAL BUTTONS (SAMA KAYA AUTO FAVORITE)
-- =========================
...
tradeSection:AddButton({
    ...
    Callback = function()
        if GlobalTrade.DataService and GlobalTrade.ItemUtility then
            local items = GlobalTrade.DataService:GetExpect({"Inventory", "Items"})
            if items and typeof(items) == "table" then
                local newFishList = {}
                ...
                for _, itemData in pairs(items) do
                    ...
                    if itemInfo and itemInfo.Data and itemInfo.Data.Type == "Fish" then
                        local fishName = itemInfo.Data.Name or "Unknown Fish"
                        if not table.find(newFishList, fishName) then
                            table.insert(newFishList, fishName)
                        end
                    end
                end
                
                table.sort(newFishList)
                GlobalTrade.FishNames = newFishList
                
                if GlobalTrade.FishDropdown then
                    GlobalTrade.FishDropdown:SetOptions(newFishList)
                end
                
                notif("Refreshed! " .. #newFishList .. " fish types", 2)
            end
        else
            notif("Services not ready", 2)
        end
    end
})

-- Reset Selection Button (SAMA KAYA AUTO FAVORITE)
tradeSection:AddButton({
    Title = "Reset Selection",
    Callback = function()
        GlobalTrade.SelectedFish = {}
        if GlobalTrade.FishDropdown then
            GlobalTrade.FishDropdown:SetValue({})
        end
        notif("Selection Reset", 2)
    end
})
-- ==================== TAB 5: WEBHOOK ====================
...
-- Function to get player list (termasuk diri sendiri)
...
local function refreshPlayerDropdown()
    ...
    local isPlayerStillOnline = false
    for _, playerName in ipairs(currentPlayers) do
        if playerName == currentSelected then
    ...
    -- Jika player yang dipilih offline, pilih LocalPlayer
    if not isPlayerStillOnline then
        selectedPlayer = LocalPlayer.Name
        playerDropdown:SetValue(LocalPlayer.Name)
    end
end

-- Create dropdown
local initialPlayers = getPlayerList()
playerDropdown = playerTeleportSection:AddDropdown({
    Title = "Select Player",
    Options = initialPlayers,
    Default = LocalPlayer.Name,
    Callback = function(v)
        selectedPlayer = v
        notif("Selected: " .. v, 2, Color3.fromRGB(0, 170, 255))
    end
})
...
playerTeleportSection:AddButton({
    ...
    Callback = function()
        ...
        if not target.Character then
            notif("Waiting for character...", 2, Color3.fromRGB(255, 170, 0))
        ...
        if not targetHRP or not myHRP then 
            notif("Character error!", 3, Color3.fromRGB(255, 0, 0))
            return 
        end
