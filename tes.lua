        ...
        if value then
            ...
            MainThread = task.spawn(function()
                ...
                while V3Config.IsRunning do
                    task.spawn(function()
                        pcall(function()
                            local cancelSuccess = net["RF/CancelFishingInputs"]:InvokeServer()
                            if not cancelSuccess then
                                while not cancelSuccess do
                                    local retry = net["RF/CancelFishingInputs"]:InvokeServer()
                                    if retry then break end
                                    task.wait(0.0)
                                end
                            end
                            
                            local chargeSuccess = net["RF/ChargeFishingRod"]:InvokeServer(tick())
                            if not chargeSuccess then
                                while not chargeSuccess do
                                    local retry = net["RF/ChargeFishingRod"]:InvokeServer(tick())
                                    if retry then break end
                                    task.wait(0.0)
                                end
                            end
                    ...
                    if not V3Config.IsRunning then break end
                    task.wait(0.1)
                end
            end)
        else
