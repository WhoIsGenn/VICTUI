...
sellSection:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num > 0 then 
            ...
            notif("Auto Sell threshold set to: " .. SellAt, 3, Color3.fromRGB(0, 255, 0))
        end
    end
})
...
sellSection:AddToggle({
    ...
    Callback = function(state)
        AutoSell = state
        if state then
            notif("Auto Sell enabled! Will sell when fish ≥ " .. SellAt, 3, Color3.fromRGB(0, 255, 0))
        else
            notif("Auto Sell disabled", 3, Color3.fromRGB(255, 0, 0))
        end
    end
})
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if fishCount >= SellAt then
        Selling = true
        notif("Selling " .. fishCount .. " fish...", 2, Color3.fromRGB(255, 255, 0))
        ...
        task.delay(1.5, function() 
            Selling = false 
            notif("Sold " .. fishCount .. " fish!", 3, Color3.fromRGB(0, 255, 0))
        end)
    end
end)

-- AUTO FAVORITE SECTION (VICTUI VERSION - NO NOTIF)
...
-- AUTO LOAD FISH DATA ON EXECUTE (SILENT)
task.spawn(function()
    local RS = game:GetService("ReplicatedStorage")
    ...
    for _, variantModule in pairs(RS.Variants:GetChildren()) do
        ...
        if ok and variantData.Data and variantData.Data.Name then
            ...
            GlobalFav.VariantIdToName[variantData.Data.Id or variantModule.Name] = variantData.Data.Name
        end
    end
    table.sort(GlobalFav.VariantNames)
    
    GlobalFav.IsLoaded = true
end)

-- Fish Selection Dropdown
local fishDropdown = favSection:AddDropdown({
    Title = "Select Fish",
...
local variantDropdown = favSection:AddDropdown({
    Title = "Select Variants",
    Content = "Loading variant data...",
    Options = {"Loading..."},
    Default = {},
    Multi = true,
    Callback = function(v)
...
task.spawn(function()
    ...
    while not GlobalFav.IsLoaded and (tick() - startTime) < timeout do
        task.wait(0.1)
    end
    
    if GlobalFav.IsLoaded then
        -- Update fish dropdown
        fishDropdown.Content = "Choose fish to auto-favorite (" .. #GlobalFav.FishNames .. " loaded)"
        fishDropdown.Options = GlobalFav.FishNames
        
        -- Update variant dropdown  
        variantDropdown.Content = "Choose variants to auto-favorite (" .. #GlobalFav.VariantNames .. " loaded)"
        variantDropdown.Options = GlobalFav.VariantNames
    end
end)
...
favSection:AddButton({
    ...
    Callback = function()
        ...
        if fishDropdown and fishDropdown.Clear then
            fishDropdown:Clear()
        end
        if variantDropdown and variantDropdown.Clear then
            variantDropdown:Clear()
        end
    end
})

-- AUTO FAVORITE LOGIC (SILENT)
task.spawn(function()
    local RS = game:GetService("ReplicatedStorage")
    ...
    REObtainedNewFishNotification.OnClientEvent:Connect(function(itemId, _, data)
        ...
        if not uuid or not fishName then 
            return 
        end

        -- Cek apakah fish dipilih
        ...
        -- Cek apakah variant dipilih
        local isVariantSelected = false
        if variantId then
            local variantName = GlobalFav.VariantIdToName[variantId]
            if variantName then
