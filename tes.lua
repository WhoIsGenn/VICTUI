...
local blatantV3Section = Tab3:AddSection("Blatant BETA")
...
--// USER CONFIG (INI YANG LU MAU)
_G.BlFishing = false
_G.ReelDelay = 0.7        -- detik
_G.CompleteDelay = 0.3    -- detik

--// INTERNAL
local reelFrame = 0
local completeFrame = 0
local frameCount = 0

local function secToFrame(sec)
    return math.max(1, math.floor(sec / (1/60)))
end

--// NO WAIT LOOP
RunService.Heartbeat:Connect(function()
    if not _G.BlFishing then return end

    frameCount += 1

    local reelEvery = secToFrame(_G.ReelDelay)
    local completeEvery = secToFrame(_G.CompleteDelay)

    -- REEL / CHARGE SPAM
    if frameCount % reelEvery == 0 then
        RF_Charge:InvokeServer(os.clock())
        RF_Start:InvokeServer(-140, 0.99)
    end

    -- COMPLETE SPAM (GA NUNGGU)
    if frameCount % completeEvery == 0 then
        RE_Complete:FireServer()
    end

    -- soft cleanup biar ga total softlock
...
BlatantV3Section:Input({
    Title = "Reel Delay (sec)",
    Default = tostring(_G.ReelDelay),
    Callback = function(v)
        local n = tonumber(v)
        if n and n > 0 then
            _G.ReelDelay = n
...
BlatantV3Section:Input({
    Title = "Complete Delay (sec)",
    Default = tostring(_G.CompleteDelay),
    Callback = function(v)
        local n = tonumber(v)
        if n and n > 0 then
            _G.CompleteDelay = n
