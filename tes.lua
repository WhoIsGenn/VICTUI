...
local fastReelSection = Tab3:AddSection("Fast Reel")
...
local GlobalFav = {
    FishNames = {},
    VariantNames = {},
    RarityNames = {"Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Secret"},
    SelectedFish = {},
    SelectedVariants = {},
    SelectedRarities = {},
...
-- Function untuk convert rarity number ke name
local function getRarityName(rarityNumber)
    for name, num in pairs(GlobalFav.RarityMapping) do
        if num == rarityNumber then
            return name
        end
    end
    return "Common"
...
task.spawn(function()
    for _, item in pairs(RS.Items:GetChildren()) do
        ...
        if ok and data.Data and data.Data.Type == "Fish" then
            ...
            table.insert(GlobalFav.FishNames, fishName)
            
            GlobalFav.FishData[fishName] = {
                Id = data.Data.Id,
                Name = fishName,
                Rarity = data.Data.Rarity or 1,
                RarityName = getRarityName(data.Data.Rarity or 1)
            }
            
            GlobalFav.FishIdToName[data.Data.Id] = fishName
            GlobalFav.FishNameToId[fishName] = data.Data.Id
        end
    end
    ...
    for _, variantModule in pairs(RS.Variants:GetChildren()) do
        ...
        if ok and variantData.Data and variantData.Data.Name then
            local variantName = variantData.Data.Name
            table.insert(GlobalFav.VariantNames, variantName)
            
            local variantId = variantData.Data.Id or variantModule.Name
            GlobalFav.VariantData[variantName] = {
                Id = variantId,
                Name = variantName
            }
            
            GlobalFav.VariantIdToName[variantId] = variantName
            GlobalFav.VariantNameToId[variantName] = variantId
        end
    end
    table.sort(GlobalFav.VariantNames)
    
    print("[Auto Favorite] Loaded " .. #GlobalFav.FishNames .. " fish types")
    print("[Auto Favorite] Loaded " .. #GlobalFav.VariantNames .. " variants")
end)
...
favSection:AddDropdown({
    ...
    Options = GlobalFav.RarityNames,
    Default = {},
    Multi = true,
    Callback = function(v)
        GlobalFav.SelectedRarities = v
        notif("Selected " .. #v .. " rarities", 3, Color3.fromRGB(170, 0, 255))
    end
})
...
-- Function untuk check apakah fish sesuai filter
local function shouldFavoriteFish(fishName, variantName, rarityNumber)
    -- Get rarity name
    local rarityName = getRarityName(rarityNumber)
    
    -- Check fish selected
    local isFishSelected = false
    if #GlobalFav.SelectedFish == 0 then
        isFishSelected = true -- Kalau ga ada selection, semua fish eligible
    else
    ...
    if #GlobalFav.SelectedVariants == 0 then
        isVariantSelected = true -- Kalau ga ada selection, semua variant eligible
    else
    ...
    -- Check rarity selected
    local isRaritySelected = false
    if #GlobalFav.SelectedRarities == 0 then
        isRaritySelected = true -- Kalau ga ada selection, semua rarity eligible
    else
        for _, selectedRarity in ipairs(GlobalFav.SelectedRarities) do
            if selectedRarity == rarityName then
    ...
    return isFishSelected and isVariantSelected and isRaritySelected
end
...
REObtainedNewFishNotification.OnClientEvent:Connect(function(itemId, _, data)
    ...
    local variantName = variantId and GlobalFav.VariantIdToName[variantId] or "Normal"
    
    -- Get fish rarity
    local fishData = GlobalFav.FishData[fishName]
    local rarityNumber = data.Rarity or (fishData and fishData.Rarity) or 1
    
    -- Check if should favorite
    if shouldFavoriteFish(fishName, variantName, rarityNumber) then
        pcall(function()
            REFavoriteItem:FireServer(uuid)
            local rarityName = getRarityName(rarityNumber)
            notif("Auto Favorited: " .. fishName .. " (" .. variantName .. ") [" .. rarityName .. "]", 3, Color3.fromRGB(0, 255, 170))
        end)
    end
end)
...
task.spawn(function()
    task.wait(1.5) -- Tunggu 1 detik biar semua element pasti udah dibuat
    LoadConfigFromFile()
end)
