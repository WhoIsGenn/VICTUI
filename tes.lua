...
local function PartOfCharacter(Inst)
    -- Only skip essential body parts from ANY character (player or NPC)
    local essentialParts = {"Head", "Torso", "UpperTorso", "LowerTorso", "HumanoidRootPart",
    ...
    -- Skip humanoid and animator from all characters
    if Inst:IsA("Humanoid") or Inst:IsA("Animator") then
        return true
    end
    
    -- Skip essential body parts only (from any character)
    if Inst:IsA("BasePart") then
        for _, partName in ipairs(essentialParts) do
            if Inst.Name == partName then
                return true
            end
        end
    end
    
    -- Allow optimization of accessories, clothing, particles from ALL characters
    return false
end

local function OptimizeInstance(Inst)
    -- Skip essential character parts only (body parts, humanoid)
    if PartOfCharacter(Inst) then
        return
    end
    ...
    pcall(function()
        ...
        -- MeshParts (skip if locked or solid modeled)
        elseif className == "MeshPart" then
            -- Skip solid modeled meshparts to avoid error
            if Inst:FindFirstChild("SolidModel") then return end
            if Inst.Locked then return end
...
local function ProcessObjectsInBatches()
    ProcessThread = task.spawn(function()
        ...
        local processed = 0
        local skipped = 0
        local batchSize = 10 -- Smaller batch for smoother performance
        local yieldEvery = 5 -- Yield every 5 batches for even smoother
        local batchCount = 0
        ...
        for i = 1, totalObjects, batchSize do
            ...
            for j = i, endIndex do
                ...
                if obj and obj.Parent then
                    -- Quick filter - only process certain types
                    local cn = obj.ClassName
                    if cn == "Part" or cn == "MeshPart" or cn == "UnionOperation" 
                    or cn == "SpecialMesh" or cn == "Decal" or cn == "Texture"
                    or cn == "ParticleEmitter" or cn == "Trail" or cn == "Smoke" 
                    or cn == "Fire" or cn == "Sparkles" or cn == "Sky" 
                    or cn == "PostEffect" or cn == "Shirt" or cn == "Pants" then
            ...
            -- Yield every few batches for smoother performance
            if batchCount >= yieldEvery then
