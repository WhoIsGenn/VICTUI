...
-- BLATANT V3 SECTION (Continuous Complete)
local blatantV3Section = Tab3:AddSection("Blatant V3")

-- V3 Config
local V3 = {
    running = false,
    reelDelay = 1.5,      -- Delay antara cycle baru
    completeInterval = 0.1, -- Interval antara complete spam
    cancelDelay = 0.08,   -- Delay cancel tetap
    mainThread = nil,
    completeThread = nil,
    isCompleteRunning = false
}

-- Reel Delay Input (delay antara cycle)
blatantV3Section:AddInput({
    ...
    Callback = function(v)
        ...
        if num and num > 0 then
            V3.reelDelay = num
            notif("Reel Delay: " .. num .. "s", 2, Color3.fromRGB(0, 255, 0))
        end
    end
})

-- Complete Interval Input (seberapa cepat complete spam)
blatantV3Section:AddInput({
    Title = "Complete Interval",
    Default = "0.1",
    Callback = function(v)
        local num = tonumber(v)
        if num and num > 0 then
            V3.completeInterval = num
            notif("Complete Interval: " .. num .. "s", 2, Color3.fromRGB(0, 255, 0))
        end
    end
})

-- Function untuk start complete spam
local function startCompleteSpam()
    if V3.isCompleteRunning then return end
    
    V3.isCompleteRunning = true
    V3.completeThread = task.spawn(function()
        while V3.isCompleteRunning do
            pcall(function()
                net["RE/FishingCompleted"]:FireServer()
            end)
            task.wait(V3.completeInterval)
        end
    end)
end

-- Function untuk stop complete spam
local function stopCompleteSpam()
    V3.isCompleteRunning = false
...
blatantV3Section:AddToggle({
    ...
    Callback = function(value)
        ...
        if value then
            -- Stop thread lama
            if V3.mainThread then 
                task.cancel(V3.mainThread) 
            end
            stopCompleteSpam()
            
            -- Start main thread
            V3.mainThread = task.spawn(function()
                while V3.running do
                    -- Pattern: Throw → Cancel → Throw → Cancel → Throw → (Complete Spam)
                    ...
                    -- START COMPLETE SPAM! (ini jalan terus)
                    startCompleteSpam()
                    
                    -- Tunggu reel delay sebelum cycle baru
                    task.wait(V3.reelDelay)
                    
                    -- Stop complete spam sebelum cycle baru
                    stopCompleteSpam()
                    
                    -- Small buffer sebelum ulang
                    task.wait(0.1)
                end
            end)
            
            notif("V3: Enabled (Continuous Complete)", 3, Color3.fromRGB(0, 255, 0))
        else
            -- Stop semua
            if V3.mainThread then
                task.cancel(V3.mainThread)
