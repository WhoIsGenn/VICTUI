...
-- ==================== FINAL - CONFIG AUTO LOAD ====================
-- Pastikan ini di PALING AKHIR setelah semua Toggle/Slider/Dropdown dibuat!

task.spawn(function()
    task.wait(2) -- Tunggu semua element siap
    
    -- Cek file system support
    if not (writefile and readfile and makefolder and isfolder) then
        notif("⚠️ Executor tidak support auto save!", 5, Color3.fromRGB(255, 165, 0))
        return
    end
    
    -- Load config
    local success = LoadConfigFromFile()
    
    if success then
        notif("✅ Config loaded successfully!", 3, Color3.fromRGB(0, 255, 0))
    else
        notif("📝 No saved config found", 3, Color3.fromRGB(255, 255, 0))
    end
